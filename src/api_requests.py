...
class BaseLocalAPIProcessor:
    """本地模型API处理器，支持自部署的LLM服务"""
    def __init__(self):
        load_dotenv()
        self.api_token = os.getenv("LOCAL_API_KEY")
        self.base_url = os.getenv("LOCAL_API_BASE_URL", "http://localhost:8000")
    ...
    def get_embeddings(self, texts, model_id="granite-embedding-278m-multilingual"):
...
class APIProcessor:
    def __init__(self, provider: Literal["openai", "local", "gemini"] ="openai"):
        self.provider = provider.lower()
        if self.provider == "openai":
            self.processor = BaseOpenaiProcessor()
        elif self.provider == "local":
            self.processor = BaseLocalAPIProcessor()
    ...
    def _build_rag_context_prompts(self, schema):
        ...
        use_schema_prompt = True if self.provider == "local" or self.provider == "gemini" else False
